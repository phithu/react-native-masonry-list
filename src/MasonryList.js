...
export default class MasonryList extends React.PureComponent {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps.layoutDimensions.width && nextProps.layoutDimensions.height &&
      ...
      this.props.layoutDimensions.height &&
    ...
    if (!this.props.rerender) {
      ...
      if (nextProps.images.length > this.props.images.length) {
        let newImages = nextProps.images.concat().
  ...
  _getCalculatedDimensions(
    imgDimensions = { width: 0, height: 0 }, columnWidth = 0, gutterSize = 0) {
  ...
  resolveImages(
    ...
    function _assignColumns(image, nColumns) {
      ...
      if (!resolverObj.columnHeightTotals[resolverObj.columnCounting - 1]) {
        ...
        resolverObj.columnHeightTotals[resolverObj.columnCounting -
        1] = resolverObj.columnHeightTotals[resolverObj.columnCounting - 1] +
          height;
      }

      if (!resolverObj.columnHighestHeight) {
        resolverObj.columnHighestHeight = resolverObj.columnHeightTotals[resolverObj.columnCounting -
        1];
        resolverObj.columnCounting = resolverObj.columnCounting < nColumns ?
          resolverObj.columnCounting + 1 :
          1;
      } else if (resolverObj.columnHighestHeight <=
      ...
      if (sorted) {
        sequence(Task, resolveImages.map((resolveTask) => {
          ...
          (err) => {
            ...
            console.warn("react-native-masonry-list", "Image failed to load.",
              err);
          },
          (resolvedImages) => {
            resolvedImages.map((resolvedData, index) => {
              ...
              let finalizedData = setItemSource(resolvedData, itemSource,
                resolvedImage);

              if (this.props.onImageResolved) {
                finalizedData = this.props.onImageResolved(finalizedData,
                  this.renderIndex) || finalizedData;
              }

              if (this.renderIndex !== 0) {
                this.setState(state => {
                  const sortedData = insertIntoColumn(finalizedData,
                    state._sortedData, sorted);
                  this._calculatedData = this._calculatedData.concat(
                    finalizedData);
                  this.renderIndex++;
                  return {
        ...
        resolveImages.map((resolveTask) => {
          if (resolveTask && resolveTask.fork) {
            return resolveTask.fork(
              (err) => {
                // eslint-disable-next-line handle-callback-err, no-console
                console.warn("react-native-masonry-list",
                  "Image failed to load.", err);
              },
              (resolvedData) => {
                const resolvedImage = getItemSource(resolvedData, itemSource);
        ...
        sequence(Task, resolveImages.map((resolveTask) => {
          ...
          (resolvedImages) => {
            resolvedImages.map((resolvedImage, index) => {
              ...
              if (this.props.onImageResolved) {
                resolvedImage = this.props.onImageResolved(resolvedImage,
                  this.renderIndex) || resolvedImage;
              }
